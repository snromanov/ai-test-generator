...
Этот документ описывает полный процесс работы с AI Test Generator для локального CLI агента (без внешних LLM).
...
Запустите локальный агент через Python REPL или однострочный скрипт:
